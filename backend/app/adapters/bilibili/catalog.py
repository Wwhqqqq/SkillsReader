"""
哔哩哔哩 Skill 多源目录抓取 —— ClawHub、SkillsMP。
"""
...
CLAWHUB_SEARCH_QUERIES = ("bilibili", "哔哩哔哩", "b站", "bilibili hot", "bilibili upload")


async def fetch_clawhub_bilibili(
    ...
    for query in CLAWHUB_SEARCH_QUERIES:
        try:
            ...
            if resp.status_code != 200:
                logger.warning("ClawHub bilibili search failed q=%s status=%s", query, resp.status_code)
                continue
            for item in resp.json().get("results") or []:
                ...
                if is_bilibili_relevant(rec):
                    records.append(rec)
        except Exception as exc:
            logger.warning("ClawHub bilibili search error q=%s: %s", query, exc)
    return await filter_records_by_vendor_relevance(vendor, records)


async def fetch_skillsmp_bilibili(
    client: httpx.AsyncClient,
    *,
    source_id: str,
    vendor: str,
