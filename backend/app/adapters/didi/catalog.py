"""
滴滴 Skill 多源目录抓取 —— ClawHub、SkillsMP。
"""
    ...
    for query in CLAWHUB_SEARCH_QUERIES:
        try:
            ...
            if resp.status_code != 200:
                logger.warning("ClawHub didi search failed q=%s status=%s", query, resp.status_code)
                continue
            for item in resp.json().get("results") or []:
                ...
                if is_didi_relevant(rec):
                    records.append(rec)
        except Exception as exc:
            logger.warning("ClawHub didi search error q=%s: %s", query, exc)
    return await filter_records_by_vendor_relevance(vendor, records)


async def fetch_skillsmp_didi(
    client: httpx.AsyncClient,
    *,
    source_id: str,
    vendor: str,
